...
if [ -r "$CREDENTIALS_DIRECTORY/gateway-env" ]; then
  ...
  SLACK_BOT=$(grep "^SLACK_BOT_TOKEN_${AU}=" "$CREDENTIALS_DIRECTORY/gateway-env" | cut -d= -f2- || true)
  [ -n "$SLACK_BOT" ] && SLACK_BOT_LINE="SLACK_BOT_TOKEN=${SLACK_BOT}"
  SLACK_APP=$(grep "^SLACK_APP_TOKEN_${AU}=" "$CREDENTIALS_DIRECTORY/gateway-env" | cut -d= -f2- || true)
  [ -n "$SLACK_APP" ] && SLACK_APP_LINE="SLACK_APP_TOKEN=${SLACK_APP}"
fi
...
# Other agents do not get this token at runtime.
case "$AGENT" in
  daen|talos)
    echo "OP_SERVICE_ACCOUNT_TOKEN=$OP_SERVICE_ACCOUNT_TOKEN" >> "$RUNTIME_DIR/env"
    echo "gcg-secret-bootstrap: $AGENT op-sa token persisted to runtime env"
    ;;
esac
...
# AWS Bedrock bearer token — Tier A agents only (bob, leon, hector, tom, phil, algaib).
# Single ABSK-prefixed key stored in 1P, region pinned to us-east-1 for cross-region inference profiles (us.anthropic.*).
case "$AGENT" in
  bob|leon|hector|tom|phil|algaib)
    BEDROCK_KEY=$(op read "op://GCG Agent Fleet/AWS Bedrock API/password" 2>/dev/null || true)
    if [ -n "$BEDROCK_KEY" ]; then
      {
        echo "AWS_BEARER_TOKEN_BEDROCK=$BEDROCK_KEY"
        echo "AWS_REGION=us-east-1"
      } >> "$RUNTIME_DIR/env"
      echo "gcg-secret-bootstrap: $AGENT bedrock bearer + region injected"
    else
      echo "gcg-secret-bootstrap: WARN $AGENT bedrock key fetch failed" >&2
    fi
    ;;
esac
